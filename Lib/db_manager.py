...
def decrypt_password(encrypted_password, encryption_key, iv):
    # Create a Cipher object with AES algorithm and CBC mode
    cipher = Cipher(algorithms.AES(encryption_key), modes.CBC(iv))
...
def add_data_to_db(data):
    ...
    cursor.execute("INSERT INTO passwords (username, password, key, iv) VALUES (?, ?, ?, ?)", data)
...
def edit_data_in_db(data):
    ...
    sql = f"UPDATE passwords SET username = ?, password = ?, key = ?, iv = ? WHERE id = {data[0]}"
